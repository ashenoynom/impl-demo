#!/usr/bin/env python3
"""
TDMS-only converter that only processes .tdms files, ignoring Excel and other files
"""
...
class TDMSOnlyConverter(TDMSBatchConverter):
    """TDMS converter that only processes .tdms files"""
    
    def find_tdms_files(self, extensions=None):
        """Override to only look for .tdms files"""
        if extensions is None:
            extensions = ['.tdms']  # Only TDMS files
    ...
    def merge_dataframes(self, dataframes, merge_strategy='concat'):
        ...
        max_rows = 200000  # Increased limit since we're only processing TDMS files
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python convert_tdms_only.py <input_folder> <output_file>")
        print("Example: python convert_tdms_only.py '/Users/ashenoy/Downloads/Hotfire 1' bzb_hotfire_tdms_only.sun")
        print("\nThis script only processes .tdms files, ignoring Excel and other files.")
        return 1
    ...
    print("Processing only .tdms files...")
    
    # Create TDMS-only converter and process
    converter = TDMSOnlyConverter(input_folder, output_file)
    
    try:
        success = converter.process_files()
        
        if success:
            print("\n🎉 TDMS conversion completed successfully!")
            summary = converter.get_summary()
            print(f"Processed {summary['total_files_processed']} TDMS files")
